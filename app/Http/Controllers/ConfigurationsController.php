...
class ConfigurationsController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('bootstrap4.admin.configuration.index', compact('configurations', 'configurations_by_sections'));
    }
    ...
    {
        return view('bootstrap4.admin.configuration.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->action('\App\Http\Controllers\ConfigurationsController@index');
    }
    ...
    public function edit($id)
    {
        ...
        return view('bootstrap4.admin.configuration.edit', compact('configuration', 'should_serialize', 'default_serialized'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->serialize) {
            ...
            if (!Auth::attempt($request->only('password'), false, false)) {
                return back()->withErrors([trans('admin.config-edit-auth-failed')]);
            }
            if (false === eval('$value = serialize('.$value.');')) {
                return back()->withErrors([trans('admin.config-edit-eval-error')]);
            }
        }

        $configuration->update(['value' => $value, 'lang' => $request->lang]);

        Session::flash('configuration', trans('lang.configuration-name-has-been-modified', ['name' => $request->name]));
        // refresh cached settings
        \Cache::forget('ticket::settings');
        \Cache::forget('ticket::settings.'.$configuration->slug);
        //return redirect(route('ticket::admin.configuration.index'));
        return redirect()->action('\App\Http\Controllers\ConfigurationsController@index');
    }
}
